...
>
    ...
    <div class="ml-header">
        <div>
            ...
            <p class="ml-sub">{{ number_format(count($mediaFiles)) }} files across {{ count($folders) }} folders</p>
        </div>
        <div class="ml-header-actions">
            <button wire:click="toggleUpload" class="ml-btn ml-btn--primary">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>
                Upload
            </button>
            <button wire:click="scanMedia" class="ml-btn ml-btn--ghost" title="Refresh">
    ...
    {{-- ========== UPLOAD ZONE ========== --}}
    <div x-show="$wire.showUpload" x-transition x-cloak class="ml-upload-zone">
        <form wire:submit.prevent="uploadMedia">
            <div class="ml-upload-area" wire:uploading>
                ...
                <input type="file" id="ml-upload-input" wire:model="uploadFile" class="ml-upload-input" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv" />
            </div>
            <div x-show="$wire.uploadFile" class="ml-upload-actions">
                <button type="submit" class="ml-btn ml-btn--primary" wire:loading.attr="disabled">
                ...
                <button type="button" wire:click="$set('uploadFile', null)" class="ml-btn ml-btn--ghost">Cancel</button>
            </div>
        </form>
    </div>
    ...
    <div class="ml-toolbar">
        <div class="ml-search">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search files by name or path…" class="ml-search-input" />
        </div>
            ...
            <div class="ml-chips">
                @foreach(['all' => 'All', 'image' => 'Images', 'document' => 'Documents'] as $key => $label)
                    <button
                        ...
                        class="ml-chip {{ $filterType === $key ? 'ml-chip--active' : '' }}"
                    >
                        {{ $label }}
                        <span class="ml-chip-count">{{ $typeCounts[$key] ?? 0 }}</span>
                    </button>
                @endforeach
            </div>
            <select wire:model.live="filterFolder" class="ml-select">
                <option value="all">All Folders</option>
                @foreach($folders as $folder)
                    <option value="{{ $folder }}">{{ $folder }}</option>
                @endforeach
            </select>
            <select wire:model.live="sortBy" class="ml-select">
    ...
    {{-- ========== FILE GRID ========== --}}
    @php $files = $this->filteredFiles; @endphp
    ...
    @else
        <div class="ml-grid">
            @foreach($files as $file)
                <div
                    class="ml-card {{ $selectedPath === $file['path'] ? 'ml-card--selected' : '' }}"
                    data-path="{{ $file['path'] }}"
                    x-data="{ path: @js($file['path']) }"
                    @click="$wire.selectFile(path)"
                >
                    <div class="ml-card-thumb">
                        ...
                        @elseif($file['type'] === 'document')
                            <div class="ml-card-icon">
                                ...
                                <span class="ml-card-ext">{{ strtoupper($file['ext']) }}</span>
                            </div>
                        @else
                            <div class="ml-card-icon">
                    ...
                    <div class="ml-card-info">
                        <div class="ml-card-name" title="{{ $file['name'] }}">{{ $file['name'] }}</div>
                        <div class="ml-card-meta">
                            {{ $file['size_human'] }}
                            @if($file['width'])<span class="ml-card-dims">{{ $file['width'] }}×{{ $file['height'] }}</span>@endif
                        </div>
                    </div>

                    <div class="ml-card-actions" @click.stop>
                        <button
                            class="ml-card-btn"
                            title="Copy path"
                            @click="$wire.copyPath(path)"
                        >
    ...
    {{-- ========== COPIED TOAST ========== --}}
    <div x-show="copied" x-transition x-cloak class="ml-toast">
    ...
    >
        @if($file = $this->selectedFile)
            <div class="ml-detail-head">
                ...
                <button wire:click="closeDetail" class="ml-detail-close">
            ...
            <div class="ml-detail-preview">
                @if($file['type'] === 'image')
                    <img src="{{ $file['url'] }}" alt="{{ $file['name'] }}" class="ml-detail-img" />
                @else
                    <div class="ml-detail-doc-icon">
                        ...
                        <span class="ml-detail-ext">{{ strtoupper($file['ext']) }}</span>
                    </div>
                @endif
            </div>

            <div class="ml-detail-name">{{ $file['name'] }}</div>

            <div class="ml-detail-meta">
                <div class="ml-detail-row"><span class="ml-detail-label">Path</span><span class="ml-detail-value ml-detail-path">{{ $file['path'] }}</span></div>
                <div class="ml-detail-row"><span class="ml-detail-label">Size</span><span class="ml-detail-value">{{ $file['size_human'] }}</span></div>
                @if($file['width'])
                    <div class="ml-detail-row"><span class="ml-detail-label">Dimensions</span><span class="ml-detail-value">{{ $file['width'] }} × {{ $file['height'] }} px</span></div>
                @endif
                <div class="ml-detail-row"><span class="ml-detail-label">Type</span><span class="ml-detail-value">{{ strtoupper($file['ext']) }} ({{ $file['type'] }})</span></div>
                <div class="ml-detail-row"><span class="ml-detail-label">Modified</span><span class="ml-detail-value">{{ $file['modified_human'] }}</span></div>
                <div class="ml-detail-row"><span class="ml-detail-label">Folder</span><span class="ml-detail-value">{{ $file['folder'] }}</span></div>
            </div>

            <div class="ml-detail-actions">
                <button @click="$wire.copyPath(@js($file['path']))" class="ml-btn ml-btn--primary ml-btn--full">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>
                    Copy Path
                </button>
                <a href="{{ $file['url'] }}" target="_blank" class="ml-btn ml-btn--ghost ml-btn--full">
                    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg>
                    Open in New Tab
                </a>
                <button
                    @click="$wire.deleteFile(@js($file['path']))"
