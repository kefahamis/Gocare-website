...
<head>
  ...
  <title>Industry Liaison &ndash; GoCare Training Institute &ndash; Kenya's Leading Healthcare College</title>
...
<nav class="nav" id="mainNav">
  ...
  <!-- -- PAGE TITLE HERO -------------------------------- -->
  <style>
    /* -- Industry Liaison Office (site theme) -- */
    .ilo-sec { padding: 80px 0; background: #fff; position: relative; overflow: hidden; }
    .ilo-sec--alt { background: linear-gradient(180deg, #faf7fc, #fff); }
    .ilo-blob { position: absolute; border-radius: 50%; filter: blur(70px); opacity: .5; pointer-events: none; z-index: 0; }
    .ilo-blob--o { background: rgba(236,116,36,.14); }
    .ilo-blob--p { background: rgba(100,42,126,.14); }
    .ilo-wrap { max-width: 1180px; margin: 0 auto; padding: 0 24px; position: relative; z-index: 2; }

    .ilo-head { text-align: center; max-width: 680px; margin: 0 auto 50px; }
    .ilo-eyebrow { display: inline-flex; align-items: center; gap: 8px; background: rgba(236,116,36,.1); color: #ec7424; padding: 7px 18px; border-radius: 50px; font-size: .74rem; font-weight: 800; text-transform: uppercase; letter-spacing: .12em; border: 1px solid rgba(236,116,36,.22); margin-bottom: 16px; }
    .ilo-eyebrow i { width: 15px; height: 15px; }
    .ilo-head h2 { font-family: 'Outfit', sans-serif; font-size: 2.4rem; font-weight: 800; color: #4a1a6d; letter-spacing: -.02em; line-height: 1.12; }
    .ilo-head h2 .grad { background: linear-gradient(120deg, #ec7424, #642a7e); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; }
    .ilo-head p { margin-top: 14px; color: #64748b; font-size: 1.05rem; line-height: 1.7; }

    .ilo-cards { display: grid; grid-template-columns: repeat(3, 1fr); gap: 26px; }
    .ilo-card { display: flex; flex-direction: column; text-decoration: none; background: #fff; border: 1px solid #efe7f3; border-radius: 20px; padding: 32px 28px; box-shadow: 0 10px 30px rgba(74,26,109,.05); position: relative; overflow: hidden; transition: transform .4s cubic-bezier(.16,1,.3,1), box-shadow .4s, border-color .3s; }
    .ilo-card::after { content: ''; position: absolute; inset: 0 0 auto 0; height: 5px; background: linear-gradient(90deg, #ec7424, #642a7e); transform: scaleX(0); transform-origin: left; transition: transform .45s cubic-bezier(.16,1,.3,1); }
    .ilo-card:hover { transform: translateY(-8px); box-shadow: 0 28px 52px rgba(74,26,109,.15); border-color: rgba(236,116,36,.35); }
    .ilo-card:hover::after { transform: scaleX(1); }
    .ilo-card-icon { width: 64px; height: 64px; border-radius: 18px; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; background: linear-gradient(135deg, #ec7424, #ff9f43); color: #fff; box-shadow: 0 10px 22px rgba(236,116,36,.3); transition: transform .45s cubic-bezier(.16,1,.3,1); }
    .ilo-card:nth-child(even) .ilo-card-icon { background: linear-gradient(135deg, #642a7e, #8e44ad); box-shadow: 0 10px 22px rgba(100,42,126,.3); }
    .ilo-card:hover .ilo-card-icon { transform: rotate(-8deg) scale(1.08); }
    ...
    .ilo-card:hover .ilo-card-link { gap: 12px; }

    /* Closing banner (matches site CTA banners) */
    .ilo-banner { background: linear-gradient(135deg, #4a1a6d 0%, #1a0a30 100%); color: #fff; text-align: center; padding: 54px 20px; position: relative; overflow: hidden; }
    .ilo-banner::before { content: ''; position: absolute; top: -80px; right: -40px; width: 280px; height: 280px; background: radial-gradient(circle, rgba(236,116,36,.25), transparent 70%); pointer-events: none; }
    .ilo-banner::after { content: ''; position: absolute; bottom: -90px; left: -50px; width: 300px; height: 300px; background: radial-gradient(circle, rgba(142,68,173,.35), transparent 70%); pointer-events: none; }
    .ilo-banner p { position: relative; z-index: 2; font-family: 'Outfit', sans-serif; font-size: 1.6rem; font-weight: 800; font-style: italic; letter-spacing: -.01em; color: #fff; }
    .ilo-banner span { color: #ff9f43; }

    /* Reveal */
    .ilo-reveal { opacity: 0; transform: translateY(26px); transition: opacity .7s cubic-bezier(.16,1,.3,1), transform .7s cubic-bezier(.16,1,.3,1); }
    .ilo-reveal.in { opacity: 1; transform: none; }

    @media (max-width: 920px) { .ilo-cards { grid-template-columns: 1fr 1fr; } .ilo-head h2 { font-size: 1.9rem; } }
    @media (max-width: 560px) { .ilo-cards { grid-template-columns: 1fr; } .ilo-banner p { font-size: 1.25rem; } }
  </style>

  <!-- PAGE HERO -->
  <section class="ph">
    <div class="ph-bg">
      <img loading="eager" decoding="async" src="images/new-images/IMG-20210922-WA0005.jpg.jpeg" alt="GoCare Industry Liaison Office">
    </div>
    ...
    <div class="ph-inner">
      <div class="ph-content">
        ...
        <nav class="ph-breadcrumb" aria-label="Breadcrumb">
          <a href="/">Home</a><i data-lucide="chevron-right"></i><span>Industry Liaison Office</span>
        </nav>
        <h1>Industry <em>Liaison Office</em></h1>
        <p>Connecting education with real-world experience through strategic partnerships and placements.</p>
        <div class="ph-btns">
          <a href="institutional-partnerships" class="ph-btn-primary">Partner With Us <i data-lucide="arrow-right"></i></a>
          <a href="industrial-attachment" class="ph-btn-ghost">Explore Internships <i data-lucide="arrow-right"></i></a>
        </div>
      </div>
    </div>
  </section>
  ...
  <!-- -- WHAT WE DO ---------------------------------------- -->
  <section class="ilo-sec" id="our-role">
    <span class="ilo-blob ilo-blob--o" style="width:320px;height:320px;top:-80px;left:-90px;"></span>
    <span class="ilo-blob ilo-blob--p" style="width:340px;height:340px;bottom:-120px;right:-90px;"></span>
    <div class="ilo-wrap">
      <div class="ilo-head ilo-reveal">
        <span class="ilo-eyebrow"><i data-lucide="briefcase"></i> What We Do</span>
        <h2>Bridging <span class="grad">Education &amp; Industry</span></h2>
        <p>The GoCare Industry Liaison Office connects learners with employers, placements, and global certifications &ndash; turning training into real careers.</p>
      </div>

      <div class="ilo-cards">
        <a class="ilo-card ilo-reveal" href="institutional-partnerships">
          <div class="ilo-card-icon"><i data-lucide="handshake"></i></div>
          <h3>Our Role</h3>
          <p>Bridging students and industry through strategic, lasting partnerships.</p>
          <span class="ilo-card-link">Learn more <i data-lucide="arrow-right"></i></span>
        </a>
        <a class="ilo-card ilo-reveal" href="industrial-attachment">
          <div class="ilo-card-icon"><i data-lucide="briefcase"></i></div>
          <h3>Internship &amp; Attachment Programs</h3>
          <p>Hands-on experience with guaranteed industrial attachment placements.</p>
          <span class="ilo-card-link">Learn more <i data-lucide="arrow-right"></i></span>
        </a>
        <a class="ilo-card ilo-reveal" href="institutional-partnerships">
          <div class="ilo-card-icon"><i data-lucide="building-2"></i></div>
          <h3>Employer Partnerships</h3>
          <p>Collaborating for success with hospitals, hotels, firms and agencies.</p>
          <span class="ilo-card-link">Learn more <i data-lucide="arrow-right"></i></span>
        </a>
        <a class="ilo-card ilo-reveal" href="careers">
          <div class="ilo-card-icon"><i data-lucide="clipboard-list"></i></div>
          <h3>Career Support &amp; Placement</h3>
          <p>Guiding your path with CV support, interview prep and job placement.</p>
          <span class="ilo-card-link">Learn more <i data-lucide="arrow-right"></i></span>
        </a>
        <a class="ilo-card ilo-reveal" href="schools/international-certifications">
          <div class="ilo-card-icon"><i data-lucide="globe"></i></div>
          <h3>International Training &amp; Certification</h3>
          <p>Globally recognised credentials through AMCA, SDC, ICDL and AHA.</p>
          <div class="ilo-card-logos">
            <img loading="lazy" decoding="async" src="images/partners/amca.png" alt="AMCA">
            <img loading="lazy" decoding="async" src="images/partners/SDCC.png" alt="SDC">
            <img loading="lazy" decoding="async" src="images/partners/ICDL.png" alt="ICDL">
            <img loading="eager" decoding="async" src="images/partners/American_Heart_Association_Logo.svg" alt="AHA">
          </div>
        </a>
        <a class="ilo-card ilo-reveal" href="home-based-care">
          <div class="ilo-card-icon"><i data-lucide="heart-pulse"></i></div>
          <h3>GoCare Health Solutions</h3>
          <p>Excellence in healthcare &ndash; professional home-based care services.</p>
          <span class="ilo-card-link">Learn more <i data-lucide="arrow-right"></i></span>
        </a>
      </div>
    </div>
  ...
  <!-- CLOSING BANNER -->
  <section class="ilo-banner">
    <p>Empowering Tomorrow's Experts Through <span>Industry Partnerships</span></p>
  </section>
  ...
  <section class="nl-sec">
    <div class="wrap nl-inner">
      ...
      <p>Get the latest programme news, intake dates and healthcare career tips &ndash; no spam, ever.</p>
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function() {
      ...
      var els = document.querySelectorAll('.ilo-reveal');
