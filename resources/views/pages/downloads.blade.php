...
<head>
  ...
  <title>Downloads | GoCare Training Institute</title>
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .dl-page { padding-top: 0; }
  .dl-hero { background: linear-gradient(135deg, var(--dark) 0%, #2d0f3c 60%, #1a0924 100%); padding: 100px 0 80px; text-align: center; position: relative; overflow: hidden; }
  .dl-hero::before { content: ''; position: absolute; inset: 0; background: radial-gradient(circle at 20% 50%, rgba(236,116,37,0.15) 0%, transparent 50%), radial-gradient(circle at 80% 30%, rgba(100,42,126,0.2) 0%, transparent 40%); pointer-events: none; }
  .dl-hero-inner { position: relative; z-index: 2; max-width: 800px; margin: 0 auto; padding: 0 20px; }
  .dl-hero-badge { display: inline-flex; align-items: center; gap: 8px; background: rgba(236,116,37,0.15); color: var(--o); padding: 8px 20px; border-radius: 50px; font-size: .85rem; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; margin-bottom: 24px; border: 1px solid rgba(236,116,37,0.25); }
  .dl-hero h1 { font-size: 3.5rem; font-weight: 800; color: #fff; line-height: 1.1; margin-bottom: 20px; }
  .dl-hero h1 span { color: var(--o); }
  .dl-hero p { font-size: 1.15rem; color: rgba(255,255,255,0.75); max-width: 600px; margin: 0 auto 32px; line-height: 1.6; }
  .dl-hero-stats { display: flex; justify-content: center; gap: 48px; margin-top: 40px; }
  .dl-stat { text-align: center; }
  .dl-stat-num { font-size: 2.2rem; font-weight: 800; color: var(--o); }
  .dl-stat-label { font-size: .85rem; color: rgba(255,255,255,0.6); font-weight: 600; margin-top: 4px; }
  .dl-section { padding: 80px 0; }
  .dl-section-header { text-align: center; margin-bottom: 56px; }
  .dl-section-header h2 { font-size: 2.4rem; font-weight: 800; color: var(--dark); margin-bottom: 12px; }
  .dl-section-header p { font-size: 1.05rem; color: #64748b; max-width: 550px; margin: 0 auto; }
  .dl-wrap { max-width: 1200px; margin: 0 auto; padding: 0 20px; }
  .dl-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 28px; }
  .dl-card { background: #fff; border-radius: 16px; overflow: hidden; border: 1px solid #e2e8f0; transition: transform .25s, box-shadow .25s; position: relative; }
  .dl-card:hover { transform: translateY(-6px); box-shadow: 0 20px 40px rgba(74,26,109,0.12); }
  .dl-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 4px; background: linear-gradient(90deg, var(--o), var(--p)); }
  .dl-card-head { padding: 32px 28px 20px; display: flex; align-items: flex-start; gap: 16px; }
  .dl-card-icon { width: 56px; height: 56px; border-radius: 14px; flex-shrink: 0; display: flex; align-items: center; justify-content: center; background: linear-gradient(135deg, rgba(236,116,37,0.1), rgba(100,42,126,0.1)); color: var(--o); }
  .dl-card-head h3 { font-size: 1.2rem; font-weight: 700; color: var(--dark); line-height: 1.3; }
  .dl-card-body { padding: 0 28px 28px; }
  .dl-card-body p { font-size: .92rem; color: #64748b; line-height: 1.6; margin-bottom: 16px; }
  .dl-card-meta { display: flex; align-items: center; gap: 8px; font-size: .8rem; color: #94a3b8; margin-bottom: 16px; }
  .dl-card-btn { display: inline-flex; align-items: center; gap: 8px; background: var(--o); color: #fff; padding: 10px 20px; border-radius: 8px; font-size: .85rem; font-weight: 700; transition: background .2s; }
  .dl-card-btn:hover { background: #d05d15; }
  .dl-additional { background: #fff; }
  .dl-add-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 24px; }
  .dl-add-card { padding: 24px; border-radius: 14px; background: var(--w); border: 1px solid #e2e8f0; display: flex; align-items: center; gap: 16px; transition: transform .2s, border-color .2s; }
  .dl-add-card:hover { transform: translateY(-4px); border-color: var(--o); }
  .dl-add-icon { width: 48px; height: 48px; border-radius: 12px; display: flex; align-items: center; justify-content: center; background: rgba(236,116,37,0.1); color: var(--o); flex-shrink: 0; }
  .dl-add-card h4 { font-size: .95rem; font-weight: 700; color: var(--dark); margin-bottom: 4px; }
  .dl-add-card span { font-size: .8rem; color: #94a3b8; }
  .dl-cta { text-align: center; padding: 80px 20px; }
  .dl-cta h2 { font-size: 2.2rem; font-weight: 800; color: var(--dark); margin-bottom: 16px; }
  .dl-cta p { font-size: 1.05rem; color: #64748b; margin-bottom: 32px; }
  .dl-cta-btn { display: inline-flex; align-items: center; gap: 10px; background: linear-gradient(135deg, var(--o), #d05d15); color: #fff; padding: 16px 40px; border-radius: 50px; font-weight: 700; font-size: 1.05rem; box-shadow: 0 10px 30px rgba(236,116,37,0.3); transition: transform .2s, box-shadow .2s; }
  .dl-cta-btn:hover { transform: translateY(-3px); box-shadow: 0 15px 40px rgba(236,116,37,0.4); }
  @media (max-width: 1024px) {
    .dl-grid { grid-template-columns: repeat(2, 1fr); }
    .dl-add-grid { grid-template-columns: repeat(2, 1fr); }
    .dl-hero h1 { font-size: 2.8rem; }
  }
  @media (max-width: 768px) {
    .dl-hero { padding: 80px 0 60px; }
    .dl-hero h1 { font-size: 2.2rem; }
    .dl-hero p { font-size: 1rem; }
    .dl-hero-stats { gap: 32px; flex-wrap: wrap; }
    .dl-stat-num { font-size: 1.8rem; }
    .dl-section { padding: 60px 0; }
    .dl-section-header h2 { font-size: 1.8rem; }
    .dl-grid { grid-template-columns: 1fr; }
    .dl-add-grid { grid-template-columns: 1fr; }
    .dl-cta h2 { font-size: 1.8rem; }
  }
  @media (max-width: 480px) {
    .dl-hero h1 { font-size: 1.8rem; }
    .dl-hero-stats { gap: 24px; }
    .dl-stat-num { font-size: 1.5rem; }
    .dl-card-head { padding: 24px 20px 16px; }
    .dl-card-body { padding: 0 20px 24px; }
    .dl-section-header h2 { font-size: 1.5rem; }
    .dl-cta-btn { padding: 14px 32px; font-size: .95rem; }
  }
</style>
<div class="dl-page">
    <!-- PAGE HERO -->
  <section class="ph">
    <div class="ph-bg">
      <img loading="eager" decoding="async" src="images/new-images/WhatsApp%20Image%202026-05-05%20at%2018.39.10.jpeg" alt="GoCare Training Institute classroom">
    </div>
    <div class="ph-overlay"></div>
    <div class="ph-inner">
      <div class="ph-content">
        <span class="ph-eyebrow"><i data-lucide="download"></i> Resources</span>
        <nav class="ph-breadcrumb" aria-label="Breadcrumb">
          <a href="/">Home</a><i data-lucide="chevron-right"></i><span>Downloads</span>
        </nav>
        <h1>Downloads & <em>Resources</em></h1>
        <p>Access the GoCare prospectus, application forms, fee structures, timetables, and other essential documents to guide your academic journey.</p>
        <div class="ph-btns">
          <a href="docs/GoCare%20Training%20Institute%20Prospectus.pdf" class="ph-btn-primary" target="_blank">Download Prospectus <i data-lucide="arrow-right"></i></a>
  ...
  <section class="dl-section"><div class="dl-wrap"><div class="dl-section-header"><h2>Key Downloads</h2><p>Essential documents every prospective and current student needs.</p></div><div class="dl-grid"><div class="dl-card"><div class="dl-card-head"><div class="dl-card-icon"><i data-lucide="book-open" style="width:28px;height:28px"></i></div><h3>Prospectus</h3></div><div class="dl-card-body"><p>Complete overview of all courses, entry requirements, fees, and campus facilities.</p><div class="dl-card-meta"><i data-lucide="file-text" style="width:14px;height:14px"></i> PDF &middot; 3.2MB</div><a href="docs/GoCare%20Training%20Institute%20Prospectus.pdf" target="_blank" class="dl-card-btn"><i data-lucide="download" style="width:16px;height:16px"></i> Download</a></div></div><div class="dl-card"><div class="dl-card-head"><div class="dl-card-icon"><i data-lucide="clipboard-list" style="width:28px;height:28px"></i></div><h3>Application Forms</h3></div><div class="dl-card-body"><p>Printable application forms for all programmes. Fill out and submit to admissions.</p><div class="dl-card-meta"><i data-lucide="file-text" style="width:14px;height:14px"></i> PDF &middot; 1.8MB</div><a href="docs/GoCare%20Training%20Institute%20Prospectus.pdf" target="_blank" class="dl-card-btn"><i data-lucide="download" style="width:16px;height:16px"></i> Download</a></div></div><div class="dl-card"><div class="dl-card-head"><div class="dl-card-icon"><i data-lucide="banknote" style="width:28px;height:28px"></i></div><h3>Fee Structure</h3></div><div class="dl-card-body"><p>Detailed fee breakdown for all programmes including payment plans and options.</p><div class="dl-card-meta"><i data-lucide="file-text" style="width:14px;height:14px"></i> PDF &middot; 950KB</div><a href="docs/GoCare%20Training%20Institute%20Prospectus.pdf" target="_blank" class="dl-card-btn"><i data-lucide="download" style="width:16px;height:16px"></i> Download</a></div></div></div></div></section>
  <section class="dl-section dl-additional"><div class="dl-wrap"><div class="dl-section-header"><h2>Additional Resources</h2><p>Supplementary forms and academic materials.</p></div><div class="dl-add-grid"><a href="docs/The%20GoCare%20Experience%20-%20Why%20You%20Should%20Join%20Us!.pdf" target="_blank" class="dl-add-card"><div class="dl-add-icon"><i data-lucide="star"></i></div><div><h4>The GoCare Experience</h4><span>PDF</span></div></a><a href="#" class="dl-add-card"><div class="dl-add-icon"><i data-lucide="clock"></i></div><div><h4>Course Timetables</h4><span>PDF &middot; 2.1MB</span></div></a><a href="#" class="dl-add-card"><div class="dl-add-icon"><i data-lucide="award"></i></div><div><h4>Graduation Lists</h4><span>PDF &middot; 800KB</span></div></a></div></div></section>
  <section class="dl-cta"><h2>Need More Documents?</h2><p>Contact our admissions team for any additional files or information you may need.</p><a href="contact" class="dl-cta-btn">Contact Us <i data-lucide="arrow-right" style="width:20px;height:20px"></i></a></section>
</div>
