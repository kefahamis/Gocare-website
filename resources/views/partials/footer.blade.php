@php
  $footerColumns = \App\Models\Menu::frontendItems('footer');
  $frontend = config('frontend.site');
  $contact = config('frontend.contact');
@endphp
...
<footer id="contactSection" class="footer">
  ...
  <div class="footer-top footer-top--five">
    ...
    <div class="footer-brand">
      ...
      <p>{{ $frontend['footer_about'] ?? 'GoCare Training Institute offers expert-led healthcare education to shape your future.' }}</p>
      <p class="footer-tagline"><em>{{ $frontend['tagline'] ?? 'Train with the Experts&hellip; Become an Expert!' }}</em></p>
    </div>
    ...
    @foreach ($footerColumns as $column)
      <div class="footer-col">
        ...
        <ul class="footer-icon-list">
          @forelse (($column['children'] ?? []) as $child)
            <li><a href="{{ ($child['url'] ?? '#') ?: '#' }}"@if (($child['target'] ?? null) === '_blank') target="_blank" rel="noopener noreferrer"@endif>{{ $child['label'] }}</a></li>
          @empty
            @if (! empty($column['url']))
              <li><a href="{{ $column['url'] }}"@if (($column['target'] ?? null) === '_blank') target="_blank" rel="noopener noreferrer"@endif>{{ $column['label'] }}</a></li>
            @endif
          @endforelse
        </ul>
      </div>
      ...
      <div class="contact-group">
        <div class="contact-icon-label">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e74c3c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <span class="contact-detail-text">{{ $contact['campuses'][0]['label'] ?? 'Nairobi City Campus' }}, {{ $contact['campuses'][0]['address'] ?? 'Nairobi CBD, GatKim Complex, Temple Road' }}</span>
        </div>
      </div>
      <div class="contact-group">
        <div class="contact-icon-label">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e74c3c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <span class="contact-detail-text">{{ $contact['campuses'][1]['label'] ?? 'Thika Road Campus' }}, {{ $contact['campuses'][1]['address'] ?? 'Eastern Bypass, Kamakis, Ruiru' }}</span>
        </div>
      </div>
      <div class="contact-group">
        <div class="contact-icon-label">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e74c3c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <span class="contact-detail-text">{{ implode(' | ', $contact['phones'] ?? ['0703 115 502', '0745 229 485', '0745 220 344']) }}</span>
        </div>
      </div>
      <div class="contact-group">
        <div class="contact-icon-label">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#e74c3c" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
            ...
            <a href="mailto:{{ $contact['email'] ?? 'info@gocareinstitute.ac.ke' }}" class="contact-detail-text" style="color:inherit;text-decoration:none">{{ $contact['email'] ?? 'info@gocareinstitute.ac.ke' }}</a>
        </div>
      </div>
    </div>
  </div>
